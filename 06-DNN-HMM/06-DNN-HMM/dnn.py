...
class FullyConnect(Layer):
    ...
    def forward(self, input):
        # BEGIN_LAB
        Z = np.dot(input, self.w.T) + self.b
        return Z
        # END_LAB
    ...
    def backward(self, input, output, d_output):
        ...
        self.dw = np.dot(d_output.T, input)        #dW = 1/m*XdZ
        self.db = np.sum(d_output, axis = 0)        #db = 1/m*sum(dZ)
...
def main():
    ...
    dnn = DNN(429, 11, 200, 1) #输入429，输出11个分类
